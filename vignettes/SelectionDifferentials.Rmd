...
bm_se_all <- data.frame(rbind(dCov = bm_booted_dCov$se, dBeforeAfter = bm_booted_dBeforeAfter$se, dMatrix = bm_booted_dMatrix$se, dReg = bm_booted_dReg$se), stringsAsFactors = FALSE)
names(bm_se_all) <- names(bm_dCov)
bm_se_all$Method = row.names(bm_se_all)


# melt the results based on the method used to compute the estimates
library(reshape2)
melt_bm <- melt(bm_all, id = "Method")
melt_bm_se <- melt(bm_se_all, id = )
...
ggplot(melt_bm, aes(Method, value, fill = Method)) + 
...
# the function can also be run without normalizing the phenotypic traits by setting normalize = FALSE
differentials(w, z, "all", normalize = FALSE)
```
