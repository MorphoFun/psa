...
# the default is to estimate the differentials with data that have been normalized to a mean of zero and unit variance
bm <- differentials(w, z, "all")

# melt the results based on the method used to compute the estimates
library(reshape2)
melt_bm <- melt(bm, id = "Method")
```
...
ggplot(melt_bm, aes(Method, value, fill = Method)) + 
  ...
  labs(title = "Selection differentials for male Bumpus datas") + 
  theme_bw() +
